...
export function CodeSurface({ children, className, padded = true }: { children: ReactNode; className?: string; padded?: boolean }) {
  return (
    <pre className={cn('m-0 overflow-auto rounded-lg bg-code font-mono text-mono text-fg-1', padded && 'px-pad py-[calc(var(--pad)-2px)]', className)}>
      {children}
    </pre>
  );
}

// Temporary TS highlighting: holds the fort until streamdown is integrated; recognizes only keywords / strings / comments / types / calls
