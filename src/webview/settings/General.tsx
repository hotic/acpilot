...
import { t } from '../i18n';
import { Field, NumberField, Section, SectionDescription, Segmented, Select, Switch } from './controls';
...
export function General({ settings, agents, on }: { settings: SettingsView; agents: AgentInfo[]; on: SettingsHandlers }) {
  ...
  const followUps = FOLLOW_UPS.map(f => ({ value: f, label: t(`settings.followUp.${f}` as const), hint: t(`settings.followUp.${f}.desc` as const) }));
  return (
    <>
      <Section>
        ...
        <Field label={t('settings.followUp')} desc={t('settings.followUp.desc')} stack>
          <div className="flex min-w-0 flex-1 flex-col gap-2">
            ...
            <div aria-live="polite"><SectionDescription>{t(`settings.followUp.${settings.followUp}.desc`)}</SectionDescription></div>
          </div>
        </Field>
      </Section>
