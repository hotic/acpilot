...
// `--ws [port]` instead runs the browser harness (test/host-preview) with one sidecar per WebSocket connection; `--home DIR`
// overrides ACPIRA_HOME for both modes

const args = process.argv.slice(2);
const flag = (name: string) => { const i = args.indexOf(name); return i >= 0 ? (args[i + 1] ?? '') : undefined; };
const home = flag('--home') ? resolve(flag('--home')!) : undefined;
...
if (args.includes('--ws')) {
  ...
  startHarness({
    port, root: process.cwd(), sessionsDir: () => sessionsDir, log: stderr,
    onWire: wire => {
      const server = new SidecarServer(wire, {
        version: VERSION, home, log: stderr,
