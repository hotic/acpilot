...
- `pnpm probe grok [--auth] [--api-key-env VAR] [--import-local] [--elicit] [prompt]` — run `initialize` + `session/new` (+ one prompt) against a CLI without VS Code; `--elicit` advertises form elicitation and prints / auto-answers `elicitation/create`. Use this first when debugging protocol issues
...
- A failed `session/prompt` does not change the session status: `AcpSession.prompt` catches the JSON-RPC error and `failTurn` puts it on the turn (`AgentTurn.stop = 'error'`, `AgentTurn.error = { message, code, kind, retryable }`); `stopReason` other than `end_turn` / `cancelled` lands on `AgentTurn.stop` too. Devin's typed cause is `data['cognition.ai/errorKind']` / `data['cognition.ai/retryable']` (the same fields as the -32015 / -32016 load errors). The webview shows the last turn's short stop as a card above the composer (`chat/Alert.tsx`, dismiss is webview-local) plus a faint closing row in the message; `retryTurn` drops both turns and re-sends the user prompt. Only `-32000` still flips the status to `auth_required`
